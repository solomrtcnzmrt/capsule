// ═══════════════════════════════════════════════════════════════
//   KAPSÜL DOLUM APARATI  —  Numara 1  (V1)
...
// ═══════════════════════════════════════════════════════════════
//   § B  NUMARA 1 KAPSÜL GEOMETRİSİ
// ═══════════════════════════════════════════════════════════════

PLATE_SIZE = 170;
N          = 10;
PLATE_T    = 11;    // 11 mm: gövde 14.3 mm → 3.3 mm taşar
CORNER_R   = 4;
CHAMFER    = 0.8;
...
PEG_OD    = BODY_ID - 0.25; // 6.18 mm  (tamper peg)
...
// Ayak (1 numara daha küçük → ayak da biraz küçük)
FOOT_D   = 16;
FOOT_H   = 10;
...
module body_plate() {
    difference() {
        ...
        four_corners() {
            ...
            translate([0, 0, -EPS])
                cylinder(d = SPR_SEAT_D, h = 3.5, $fn = 48);
        }
        align_holes();
    }
}
...
module middle_plate() {
    STEP_H   = 4.0;
    SEP_HOLE = BODY_OD + 0.20;  // 6.83 mm (size 1)
    difference() {
        base_plate(PLATE_T);
        for (r = [0:N-1], c = [0:N-1])
            translate([BORDER + c*PITCH, BORDER + r*PITCH, 0]) {
...
if      (PART == "body_plate")    body_plate();
    ...
    translate([  G, 2*G, 0]) {
        for (i=[0:3]) translate([i*24, 0, 0]) foot();
    }
    translate([G+100, 2*G, 0]) tpu_spring();
    translate([G+130, 2*G, 0]) alignment_pin();
}
